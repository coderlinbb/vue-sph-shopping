          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="index">
                ...
                <dd changepirce="0" @click="changeActive(value,spuSaleAttr.spuSaleAttrValueList)" :class="{active:value.isChecked == 1}" v-for="value in spuSaleAttr.spuSaleAttrValueList" :key="value.id">{{value.saleAttrValueName}}色</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  methods: {
    ...
    async addShopcart() {
      try {
        let a = await this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        console.log(a)
      } catch (error) {
        alert(error.message)
      }
